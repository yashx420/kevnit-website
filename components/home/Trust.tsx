...
import { Cloud, BarChart3, Share2, Workflow, Target, Facebook, Activity } from "lucide-react";
...
export function Trust() {
  return (
    <section className="py-10 bg-black border-y border-white/5 overflow-hidden">
      <Container>
        <p className="text-center text-gray-500 text-xs font-semibold tracking-widest mb-8 uppercase">Powered by Trusted Global Platforms</p>
          ...
          >
            {[...companies, ...companies].map((company, i) => (
              <div key={i} className="flex items-center gap-2 group cursor-default">
                <company.icon className={`w-8 h-8 ${company.color} opacity-70 group-hover:opacity-100 transition-opacity`} />
